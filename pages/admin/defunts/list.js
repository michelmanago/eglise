...
export default function Defunts({ defunts }) {
    const router = useRouter()
    const { locale } = router
    const [defuntsList, setDefuntsList] = useState(defunts)

    const dataEndpoint = (offset, limit) => `/admin/defunts?offset=${offset}&limit=${limit}`
                            ...
                            <div className="flex justify-center">
                                {defuntsList.metadata.offset !== 0 ?
                                    <div className={styles.btn_nav}
                                        ...
                                        data-limit={defuntsList.metadata.limit} onClick={onClickPrev}>
                                        {"<"}
                                    </div>
                                    :
                                ...
                                {defuntsList.data.map((defunt, i) => (
                                    <div className="flex w-1/2 sm:w-1/3 lg:w-1/4 xl:w-1/5" key={defunt.id}>
                                        <div className="flex-1 p-2 mb-2 ml-2 border border-black rounded" key={i}>
                                            <Link href={`/admin/defunts/${defunt.id}`}>
                                                <a>{defunt.prenom} {defunt.nom}</a>
                                            </Link>
                                        </div>
                                    </div>
                                ))}
...
/*export async function getServerSideProps(context) {
    ...
    if (req) {
        ...
        //const res = await fetch(`http://localhost:3000/admin/defunts`)
        const res = await fetch(`${protocol}${host}/admin/defunts`)
