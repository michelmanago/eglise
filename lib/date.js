...
export function formatDateToDisplayByLocale(date, locale) {
    var d = new Date(date),
        month = '' + (d.getMonth() + 1),
        day = '' + d.getDate(),
        year = d.getFullYear();
    ...
    if (locale === 'en') return [month, day, year].join('/');
...
export function formatDateToInput(date) {
    ...
    if (isValidDate(jsDate)) return jsDate.toISOString().slice(0, 10);
    else return null;
}
